...
@Component
public class QuotePublisher2TopicExchange {

	/** The logger */
	private static final Logger LOGGER = Logger.getLogger(QuotePublisher2TopicExchange.class);

	private static final String TOPIC_EXCHANGE = "topic_asset_exchange";
	 ...
	 * @param amqpTemplate
	 */
	public QuotePublisher2TopicExchange(AmqpAdmin amqpAdmin, AmqpTemplate amqpTemplate) {
		this.amqpAdmin = null;
	...
	public void publish() {
		...
		for (int i=0; i<10; i++){
			...
			String routingKey = "asset."+asset.getType().toLowerCase();

			amqpTemplate.convertAndSend(TOPIC_EXCHANGE, routingKey, asset.toString());
